...
int main(int argc, char **argv)
{
    ...
    send_tmp = (double**)malloc(sizeof(double*) * rp * rp * 2);
    ...
    if (myid == 0)
    {
        ...
        for (i = 0; i < rp; i++)
        {
            for (j = 0; j < rp; j++)
            {
                ...
                if (target != 0)
                {
                    MPI_Isend(send_tmp[req_count], buff_sizeA, MPI_DOUBLE, target, 0, MPI_COMM_WORLD, &(req[req_count]));
                    req_count++;
                }
                int b_i = i - j;
                if (b_i < 0)
                ...
                if (target != 0)
                {
                    MPI_Isend(send_tmp[req_count], buff_sizeB, MPI_DOUBLE, target, 0, MPI_COMM_WORLD, &(req[req_count]));
                    req_count++;
                }
            }
        }
